...
export async function validateTemplateWithLLM(
  ...
  const template =
    ...
    TEMPLATE_DOCTYPE["OTHER_DOCUMENT"];

  const system = `
You are a strict document validator.

Compare OCR_TEXT against TEMPLATE.

Return JSON ONLY.

SPECIAL RULES FOR RDB_CERTIFICATE:

1. HEADER elements ("RWANDA", "DEVELOPMENT BOARD", "ORG", "Office of the Registrar General") may appear in any order.
2. Seal valid if "Serial No" and "rdb.rw" exist.
3. Accept Date format DD/MM/YYYY.
